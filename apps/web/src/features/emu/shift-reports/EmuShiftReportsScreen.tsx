...
function ShiftReportsFeature({ currentUser, onNotify, screen }: { currentUser: SessionUserDto | null; onNotify: (message: string) => void; screen: ShiftReportScreen }) {
  ...
  return isHistory ? <ShiftReportHistoryScreen workspace={workspace} /> : <ShiftReportEntryScreen workspace={workspace} currentUserId={currentUser.id} onNotify={onNotify} canManageFavorites={hasPermission(currentUser, 'emu.favorite-employees.manage') || hasPermission(currentUser, 'emu.shift-reports.create')} />;
}
