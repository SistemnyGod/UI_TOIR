      ...
      <header className="inventory-overview-commandbar">
        <div className="inventory-overview-title">
          ...
          <div>
            <p>Бухгалтерия</p>
            <h1>Обзор учета</h1>
            <span>Выдачи, возвраты, списания, СИЗ, под запись и контроль движения предметов.</span>
          </div>
        </div>
      ...
      {!loading && !error && overview ? (
        <>
          <section className="inventory-overview-kpis" aria-label="Сводка учета">
            ...
            <OverviewKpi icon={PackageCheck} label="Учетные группы" value={overview.warehousesTotal} />
            <OverviewKpi icon={AlertTriangle} label="Зоны внимания" tone={overview.criticalStockItems ? "red" : "green"} value={overview.criticalStockItems} />
          ...
          <section className="inventory-overview-workspace">
            <section className="inventory-overview-main">
              ...
              {!overview.attention.length ? (
                <OverviewState kind="empty" title="Критичных зон нет" text="Просрочек, незакрытых движений и аномальных операций сейчас нет." compact />
              ) : (
