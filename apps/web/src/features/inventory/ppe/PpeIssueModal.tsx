  ...
  return (
    <div className="ppe-v2-modal-backdrop" onMouseDown={onClose} role="presentation">
      <section aria-label="Выдать СИЗ" className="ppe-v2-modal ppe-v2-issue-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <button className="ppe-v2-item-choice" onClick={onChooseItem} type="button">
          <span><small>Номенклатура</small><strong>{item?.name ?? "Не сопоставлено"}</strong></span><Link2 size={18} />
        </button>
        ...
        <footer className="ppe-v2-modal-actions"><button className="button" onClick={onClose} type="button">Отмена</button><button className="button primary" disabled={saving} onClick={() => void submit()} type="button">{saving ? "Сохранение…" : "Подтвердить выдачу"}</button></footer>
      </section>
    </div>
  );
}
