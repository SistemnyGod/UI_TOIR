  ...
  return (
    <PpeModalShell
      ...
      footer={(
        <>
          ...
          <PpeButton disabled={!item} loading={saving} onClick={() => void submit()} variant="primary">
    ...
    >
      ...
      <button className={"ppe-v2-item-choice " + (item ? "is-selected" : "is-required")} onClick={onChooseItem} type="button">
        <span><small>{item ? "Выбрано по норме" : "Требуется выбор"}</small><strong>{item?.name ?? "Выберите допустимую позицию СИЗ"}</strong>{item ? <em>{[item.sku, item.article, item.unit].filter(Boolean).join(" · ")}</em> : null}</span>
      ...
      <div className="ppe-v2-form-grid">
        ...
        <label>Дата выдачи<input data-ppe-initial-focus max="2999-12-31" onChange={(event) => setIssuedAt(event.target.value)} type="date" value={issuedAt} /></label>
