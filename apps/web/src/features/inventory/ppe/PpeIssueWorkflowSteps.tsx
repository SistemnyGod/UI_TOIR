  ...
  return (
    <section className="ppe-issue-card ppe-issue-selection-card">
      ...
      <div className="ppe-issue-toolbar">
        ...
        <div className="ppe-issue-toolbar-actions"><PpeButton icon={<Plus size={16} />} onClick={onAddCatalog} variant="secondary">Добавить из каталога</PpeButton>{selectionTab === "norms" ? <PpeButton disabled={!mappedCount} icon={<Check size={16} />} onClick={onSelectAll} variant="secondary">Отметить сопоставленные</PpeButton> : null}</div>
      </div>
...
export function PrintStep({ committed, data, errors, mode, onDownload, onModeChange, onPreview, onPrint, onSave, saving }: { committed: boolean; data: PrintData; errors: string[]; mode: PrintMode; onDownload: (format: "pdf" | "docx") => void; onModeChange: (mode: PrintMode) => void; onPreview: () => void; onPrint: () => void; onSave: () => void; saving: boolean }) {
  return <section className="ppe-issue-card ppe-issue-print-card"><CardHeading icon={<Printer size={24} />} kicker="Шаг 4" title="Печать и предпросмотр" text="Проверьте реальную форму документа перед сохранением выдачи." />{errors.length ? <div className="ppe-issue-print-warning" role="alert"><strong>Документ пока нельзя сохранить</strong><ul>{errors.map((error) => <li key={error}>{error}</li>)}</ul><span>Предпросмотр и экспорт доступны для проверки макета.</span></div> : null}<div className="ppe-issue-print-toolbar"><div className="ppe-issue-print-tabs" role="tablist"><Tab active={mode === "card"} label="Личная карточка" onClick={() => onModeChange("card")} /><Tab active={mode === "sheet"} label="Лист выдачи" onClick={() => onModeChange("sheet")} /></div><div><PpeButton onClick={onPreview} variant="secondary">Открыть крупно</PpeButton><PpeButton icon={<Printer size={15} />} onClick={onPrint} variant="secondary">Печать</PpeButton><PpeButton icon={<FileText size={15} />} onClick={() => onDownload("pdf")} variant="secondary">PDF</PpeButton><PpeButton icon={<FileText size={15} />} onClick={() => onDownload("docx")} variant="secondary">DOCX</PpeButton></div></div><div className="ppe-issue-print-paper-wrap"><PrintPaper data={data} mode={mode} /></div><div className="ppe-issue-save-panel"><div><strong>{committed ? "Документ выдачи сохранён" : "Готовы сохранить выдачу?"}</strong><span>{committed ? "Запись добавлена в историю сотрудника." : "Финальное сохранение создаст все факты выдачи одной операцией."}</span></div><PpeButton disabled={committed || errors.length > 0} icon={<CheckCircle2 size={16} />} loading={saving} onClick={onSave} variant="primary">{committed ? "Сохранено" : "Сохранить документ выдачи"}</PpeButton></div></section>;
}
...
function SetsStart({ settings, onApply }: { settings: InventorySettingsDto | null; onApply: (set: InventoryItemSetDetailDto) => Promise<void> }) {
  ...
  async function open(id: string) { setLoadingId(id); setError(""); try { setDetail(await repository.getItemSet(id)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось загрузить набор"); } finally { setLoadingId(""); } }
  async function apply() { if (!detail) return; setSaving(true); setError(""); try { await onApply(detail); setDetail(null); } catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось применить набор"); } finally { setSaving(false); } }
  return <div className="ppe-issue-catalog-start"><div className="ppe-issue-catalog-hero"><Layers3 size={28} /><div><h3>Наборы СИЗ</h3><p>Просмотрите состав перед добавлением. Дублирующиеся товары не создаются повторно.</p></div></div>{error ? <div className="ppe-issue-error" role="alert">{error}</div> : null}{sets.length ? <div className="ppe-issue-set-list">{sets.map((set) => <article key={set.id}><div><strong>{set.name}</strong><span>{set.itemsCount} позиций</span></div><PpeButton disabled={Boolean(loadingId) && loadingId !== set.id} loading={loadingId === set.id} onClick={() => void open(set.id)} variant="secondary">Посмотреть</PpeButton></article>)}</div> : <div className="ppe-issue-empty-inline"><Layers3 size={19} /> Наборы ещё не настроены</div>}{detail ? <section className="ppe-issue-set-preview"><header><div><strong>{detail.name}</strong><span>{detail.items.length} позиций</span></div><PpeButton aria-label="Закрыть состав набора" onClick={() => setDetail(null)} variant="link">Закрыть</PpeButton></header><ul>{detail.items.map((line) => <li key={line.id}><span><strong>{line.item.name}</strong><small>{[line.item.article, line.item.category].filter(Boolean).join(" · ")}</small></span><b>{line.quantity} {line.item.unit}</b></li>)}</ul><PpeButton loading={saving} onClick={() => void apply()} variant="primary">Добавить набор в выдачу</PpeButton></section> : null}</div>;
}
