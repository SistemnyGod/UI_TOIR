...
const makeIcon =
  (symbol: string): IconComponent =>
  ({ size = 18 }) => (
    <span className="assign-am-glyph" style={{ fontSize: `${Math.max(12, size - 2)}px` }} aria-hidden="true">
      {symbol}
    </span>
  );
...
const defaultAssignmentShiftSettings: ShiftTimeSettings = {
  dayEnd: "14:00",
  dayStart: "06:00",
  nightEnd: "06:00",
  nightStart: "22:00",
...
export function AssignmentScreen({
  ...
  const defaultPeriodDate = toDateInput(new Date(Date.now() + 24 * 60 * 60 * 1000));
  ...
  async function handleAssign() {
    ...
    try {
      ...
      if (selectedRequest || dataSourceMode !== "api") {
...
function FilterBox({ icon: Icon, label, value }: { icon?: IconComponent; label: string; value: string }) {
  return (
  ...
  const activeRequestItems = [
    ...activeAssignments.map((assignment) => {
      ...
      return {
        ...
        meta: assignment.startedAt || assignment.eta || "сейчас",
            ...
            <div className="assign-am-history-list">
              ...
              {historyEvents.map((event) => (
                <div className="assign-am-history-event" key={event.id}>
                  ...
                  <small>{event.status} · {event.meta}</small>
                </div>
              ))}
            </div>
          ) : (
...
function ActiveAssignmentsCard({
  ...
  return (
    <section className="assign-am-card">
      <PanelHeader count={assignments.length} icon={CheckCircle2} title="Активные назначения" />
      {status === "loading" ? (
        ...
        <div className="assign-am-table">
          <div className="head"><span>Сотрудник</span><span>Маршрут</span><span>Начало</span><span>Статус</span><span>Прогресс</span></div>
          {assignments.map((assignment) => (
            <div className="row" key={assignment.id}>
              <strong>{assignment.employee}</strong>
              <span>{assignment.route}</span>
              <span>{assignment.startedAt || assignment.eta}</span>
              <StatusPill value={assignment.status} />
              <span className="assign-am-progress"><i style={{ width: `${assignment.progress}%` }} /></span>
              <div className="actions">
                <button disabled={!canManage || savingAssignmentId === assignment.id} onClick={() => void onRunCommand(assignment.id, "start")} type="button">Старт</button>
                <button disabled={!canManage || savingAssignmentId === assignment.id} onClick={() => void onRunCommand(assignment.id, "complete")} type="button">Готово</button>
                <button disabled={!canManage || savingAssignmentId === assignment.id} onClick={() => void onRunCommand(assignment.id, "cancel")} type="button">Отмена</button>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <EmptyPanel description="Назначения появятся после отправки заявки сотруднику." title="Активных назначений нет" />
      )}
    </section>
  );
}
    ...
    <div className="assign-am-modal-backdrop" onClick={onClose}>
      <section className="assign-am-complete-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
        ...
        <div className="assign-am-complete-summary">
          ...
          <small>{assignment.startedAt || assignment.eta}</small>
        </div>
...
function StatusPill({ value }: { value: string }) {
  ...
  const tone = text === "Просрочено" ? "danger" : text === "Проверка" ? "warning" : "success";
  return <span className={`assign-am-status ${tone}`}>{text}</span>;
}
...
function loadAssignmentFavoriteEmployeeIds() {
...
function saveAssignmentFavoriteEmployeeIds(ids: string[]) {
  if (typeof window === "undefined") return;
...
function loadAssignmentShiftSettings(): ShiftTimeSettings {
...
function saveAssignmentShiftSettings(settings: ShiftTimeSettings) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(assignmentShiftSettingsStorageKey, JSON.stringify(normalizeShiftSettings(settings)));
}

function normalizeShiftSettings(settings: Partial<ShiftTimeSettings>): ShiftTimeSettings {
  return {
...
function shiftText(value: string) {
  return isNightShift(value) ? "3 смена" : "1 смена";
}

function shiftTime(value: string, settings: ShiftTimeSettings) {
...
function shiftStartTime(value: string, settings: ShiftTimeSettings) {
...
function isNightShift(value: string) {
  return value.includes("Н") || value === "Ночь";
}

function employeeStatusText(value: string) {
...
function assignmentStatusText(value: string) {
  if (value.includes("Зад") || value === "Задержка") return "Просрочено";
  if (value.includes("Зав") || value === "Завершает") return "Проверка";
  if (value.includes("Ож") || value === "Ожидает") return "Ожидает";
  return "Выполняется";
}

function isAssignmentCurrent(assignment: ActivePatrol) {
  const status = assignment.status.toLowerCase();
  return !status.includes("заверш") && !status.includes("отмен") && !status.includes("закры");
}

function isRequestCurrent(request: ServiceRequest) {
...
function isAssignableRequest(request: ServiceRequest) {
