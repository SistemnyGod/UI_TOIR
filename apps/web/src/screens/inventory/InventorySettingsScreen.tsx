        ...
        <>
          ...
          {tab === "references" ? (
            <section className="inventory-settings-reference-grid">
              ...
              <ReferenceCard kind="warehouse" rows={effectiveSettings.warehouses} title="Склады" onCreate={() => setReferenceModal({ kind: "warehouse" })} onEdit={(row) => setReferenceModal({ kind: "warehouse", row })} onToggle={(row) => void toggleReference("warehouse", row)} />
...
function referenceHint(kind: ReferenceKind) {
  const hints: Record<ReferenceKind, string> = {
    ...
    warehouse: "Места учета остатков",
...
function healthTitle(issue: InventoryDbHealthIssueDto) {
  const map: Record<string, string> = {
    "inventory.stock.negative": "Отрицательные остатки",
    ...
    "legacy.warehouses.synthetic_names": "Синтетические склады",
  };
...
function healthDescription(issue: InventoryDbHealthIssueDto) {
  const map: Record<string, string> = {
    "inventory.stock.negative": "Есть позиции, где расчетный остаток ниже нуля.",
    ...
    "legacy.warehouses.synthetic_names": "Остались склады-заглушки из legacy-import.",
  };
