...
export function InventoryReportsScreen({ error, loading = false, onNotify, reports }: InventoryReportsScreenProps) {
  ...
  const visibleReports = useMemo(() => {
    ...
    return rows.filter((report) =>
      !normalized ||
      [report.title, report.description, report.id, report.format]
        .join(" ")
        .toLowerCase()
        .includes(normalized),
    );
  ...
  async function exportReport(report: InventoryReportDto, format: ExportFormat) {
    ...
    try {
      ...
      onNotify(`Отчет «${report.title}» сформирован`);
    } catch (downloadError) {
      ...
      <header className="inventory-reports-commandbar">
        <div className="inventory-reports-title">
          ...
          <div>
            <p>Бухгалтерия</p>
            <h1>Отчеты</h1>
            <span>Выгрузки по остаткам, движениям, СИЗ, под запись, сотрудникам, истории и системному журналу.</span>
          </div>
        </div>
      </header>
              ...
              {visibleReports.map((report) => {
                ...
                return (
                  <article className="inventory-reports-card" key={report.id}>
                    ...
                    <div>
                      <h2>{report.title}</h2>
                      <p>{report.description || "Описание отчета не задано"}</p>
                    </div>
