...
export function useCustodyRepositoryActions({
    ...
    try {
      ...
      setDrawer({
        ...
        meta: [
          ...
          ["Склад", row.warehouseName || "Не указан"],
  ...
  async function updateRecordStatus(row: InventoryCustodyRecordDto, status: string, documentId?: string) {
    try {
      setBusyAction(`${status}-${row.id}`);
      await inventoryRepository.updateCustodyRecordStatus(row.id, { status });
      onNotify("Статус строки под запись обновлен");
      await onReload();
