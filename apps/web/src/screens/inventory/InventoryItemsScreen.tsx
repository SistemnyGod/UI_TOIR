    ...
    <section className="inventory-items-screen">
      <header className="inventory-items-commandbar">
        <div className="inventory-items-title-block">
          ...
          <div>
            <h1>Номенклатура</h1>
            <p>Позиции, категории, единицы учета, остатки и карточки для выдачи.</p>
          </div>
        </div>
                ...
                {!loading
                  ? items.rows.map((item) => (
                      ...
                      >
                        ...
                        <td>{formatQuantity(item.stockAvailable, item.unit)}</td>
                        <td>
    ...
    return (
      <aside className="inventory-item-inspector">
        <div className="inventory-inspector-empty">
          ...
          <p>Карточка справа покажет остатки, параметры и быстрые действия.</p>
        </div>
      </aside>
    );
  }
  ...
  return (
    <aside className="inventory-item-inspector">
      ...
      <dl className="inventory-inspector-list">
        ...
        <div>
          <dt>Остаток склада</dt>
          <dd>{formatQuantity(item.stockAvailable, item.unit)}</dd>
        </div>
        <div>
          <dt>На руках</dt>
          <dd>{formatQuantity(item.balance - item.stockAvailable, item.unit)}</dd>
        </div>
      </dl>
...
function ItemFormDialog({
  ...
  return (
    <div className="inventory-dialog-backdrop" role="presentation">
      <form className="inventory-dialog inventory-item-form-dialog" onSubmit={handleSubmit}>
        <DialogHead
...
function ItemDetailDialog({
  ...
  return (
    <div className="inventory-dialog-backdrop" role="presentation">
      <article className="inventory-dialog inventory-detail-dialog">
        ...
        <div className="inventory-detail-grid">
          ...
          <DetailStat icon={Boxes} label="Остаток" value={formatQuantity(item.stockAvailable, item.unit)} />
...
function ConfirmHideDialog({
  ...
  return (
    <div className="inventory-dialog-backdrop" role="presentation">
      <article className="inventory-dialog inventory-confirm-dialog">
...
function formatMoney(value: number | null | undefined) {
  if (value === null || value === undefined) return "0,00 в‚Ѕ";
