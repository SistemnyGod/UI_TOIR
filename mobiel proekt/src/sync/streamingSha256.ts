...
export class StreamingSha256 {
  ...
  digestHex() {
    ...
    for (let index = 0; index < 8; index += 1) {
      padding[padding.length - 1 - index] = Math.floor(bitLength / 2 ** (index * 8)) & 0xff;
    }
  ...
  private updatePadding(bytes: Uint8Array) {
    let offset = 0;
    if (this.blockLength > 0) {
      const copied = Math.min(64 - this.blockLength, bytes.byteLength);
      this.block.set(bytes.subarray(0, copied), this.blockLength);
  ...
  private compress(block: Uint8Array) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const smallSigma0 = (rotateRight(value, 7) ^ rotateRight(value, 18) ^ (value >>> 3)) >>> 0;
      const next = words[index - 2];
      const smallSigma1 = (rotateRight(next, 17) ^ rotateRight(next, 19) ^ (next >>> 10)) >>> 0;
