...
export async function countPendingOutboxCommands() {
  const db = await getDatabase();
...
export async function hasPendingOutboxCommands() {
  return (await countPendingOutboxCommands()) > 0;
}

export async function listSyncQueueCommands(limit = 100) {
  const db = await getDatabase();
  const rows = await db.getAllAsync<{
    client_operation_id: string;
    command_type: string;
    ...
    `
      ...
      WHERE command.status IN ('pending', 'sending', 'retryLater', 'rejected', 'conflict')
      ORDER BY
        CASE command.status
          WHEN 'sending' THEN 0
...
export async function listUnconfirmedCompleteReportCommands(assignmentId?: string) {
  const db = await getDatabase();
...
export async function markPendingOutboxCommandsRetryLater(lastError: string) {
  const db = await getDatabase();
  const updatedAtLocal = new Date().toISOString();
  ...
  await db.runAsync(
    `
      ...
      WHERE status IN ('pending', 'sending', 'retryLater')
    `,
    [lastError, updatedAtLocal]
  );
}

export async function markPendingOutboxCommandsAuthRequired(lastError: string) {
  const db = await getDatabase();
  const updatedAtLocal = new Date().toISOString();
  ...
  await db.withExclusiveTransactionAsync(async (tx) => {
    ...
    await tx.runAsync(
      `
        ...
        WHERE status IN ('accepted', 'inProgress', 'paused', 'completedLocal', 'syncing', 'syncError')
          AND EXISTS (
            SELECT 1
            FROM outbox_commands command
...
export async function finalizeAcceptedCompleteReportCommands(assignmentId?: string) {
  const db = await getDatabase();
  let finalized = 0;
    ...
    const rows = await tx.getAllAsync<{ assignment_id: string }>(
      `
        ...
        INNER JOIN patrol_assignments assignment
          ON assignment.assignment_id = command.entity_local_id
        WHERE command.command_type = 'completePatrolAssignment'
